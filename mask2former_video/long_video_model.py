...
class LongVideo_inference_model(nn.Module):
    ...
    def __call__(self, inputs, num_clips=2):
        '''
    ...
    def _inference_clips(self, clips_in: list):
        '''
    ...
    def _merge_outputs(self, out_before, out_now, dis_thr=5):
        ...
        em_frame = torch.full((out_before['pred_masks'][0][0].shape[0], out_before['pred_masks'][0][0].shape[1]), False)
        ...
        for k1, v1 in out1.items():
            ...
            box_center_dist = np.zeros((len(v1['masks']), len(v2['masks'])))
            for mind1, m1 in enumerate(v1['masks']):
                boxes1 = masks_to_boxes(m1)
                centr1 = np.array(np.mean(boxes1[-1][0::2]), np.mean(boxes1[-1][1::2]))  # last clip's last frame
                for mind2, m2 in enumerate(v2['masks']):
                    boxes2 = masks_to_boxes(m2)
                    centr2 = np.array(np.mean(boxes2[0][0::2]), np.mean(boxes2[0][1::2]))  # current clip's first frame
                    box_center_dist[mind1, mind2] = 0.05 * np.linalg.norm(centr1 - centr2) + 0.95 * np.linalg.norm(
                        v1['qurry_feature'][mind1] - v2['qurry_feature'][mind2])

            box_center_dist[np.where(box_center_dist > 65)] = 0

            match1 = np.zeros(box_center_dist.shape[0])
            match2 = np.zeros(box_center_dist.shape[1])
            m_dis = np.array(np.where(box_center_dist > 0)).T  # [[x,y],[]...]
            dis_value = [box_center_dist[_v[0], _v[1]] for _v in m_dis]
            top_index = np.argsort(np.array(dis_value))
        ...
        indx = np.argsort(-np.array(final_out["pred_scores"]))[:10]

        final_out["pred_scores"] = list(np.array(final_out["pred_scores"])[indx])
        final_out["pred_masks"] = [final_out["pred_masks"][_m] for _m in indx]
        final_out["pred_labels"] = list(np.array(final_out["pred_labels"])[indx])
        # print(len(final_out["pred_labels"]), len(final_out["pred_masks"]), len(final_out["pred_scores"]))
        # sys.exit()
        return final_out
    ...
    def _divide_videos(self, video_in, num):
        ...
        clip_list = []
        video = video_in.pop('image')
        for i in range(num):
            clip_i = copy.deepcopy(video_in)
            if i != num - 1:
                clip_i['image'] = video[i:i + len_clip]
        ...
        print(video_in['length'])
        print([len(_c['image']) for _c in clip_list])
        return clip_list
...
def masks_to_boxes(masks_in):
    ...
    """
    masks = torch.as_tensor(masks_in)
    if masks.numel() == 0:
    ...
    return np.array(torch.stack([x_min, y_min, x_max, y_max], 1))
