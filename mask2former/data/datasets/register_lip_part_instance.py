...
_PREDEFINED_SPLITS = {
    "lip_part_instance_train":("lip/Training/Images/", "lip/annotations/LIP_part_instance_train.json",),
    "lip_part_instance_val":("lip/Validation/Images/", "lip/annotations/LIP_part_instance_val.json",),
}


def _get_lip_part_instances_meta():
    thing_ids = [k["id"] for k in LIP_PART_INSSEG_CATEGORIES]
    assert len(thing_ids) == 19, len(thing_ids)
    # Mapping from the incontiguous CIHP category id to an id in [0, 99]
    thing_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(thing_ids)}
    thing_classes = [k["name"] for k in LIP_PART_INSSEG_CATEGORIES]
    ret = {
...
def register_all_lip_part_instance(root):
    for key, (image_root, json_file) in _PREDEFINED_SPLITS.items():
        # Assume pre-defined datasets live in `./datasets`.
        register_coco_instances(
            key,
            _get_lip_part_instances_meta(),
...
register_all_lip_part_instance(_root)
