...
from .data.parsing_insseg_utils import get_parsing_flip_map
...
class ParsingSemanticSegmentorWithTTA(nn.Module):
    ...
    def __init__(self, cfg, model, tta_mapper=None, batch_size=1):
        ...
        if isinstance(model, DistributedDataParallel):
            model = model.module
        self.cfg = cfg.clone()
        self.flip_map = get_parsing_flip_map(cfg.DATASETS.TRAIN)
    ...
    def flip_parsing_back(self, predictions):
        ...
        for ori_label, new_label in self.flip_map:
            org_channel = spatial_flipback_predictions[ori_label + 1, :, :]
            new_channel = spatial_flipback_predictions[new_label + 1, :, :]

            spatial_channel_flipback_predictions[new_label + 1, :, :] = org_channel
            spatial_channel_flipback_predictions[ori_label + 1, :, :] = new_channel
